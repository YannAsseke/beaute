    ...
    <>
      <li
        className={`${customClass ? customClass : ""} ${
          menu?.badge ? "new-nav-item" : ""
        } ${menu.children ? "dropdown-mega" : ""}`}
      >
        {menu?.path ? (
          <Link
            className={`${
              anchorClass ? anchorClass : "nav-link dropdown-toggle"
            }`}
            href={`/${i18Lang}/collections?category=${menu?.path}`}
          >
            {t(menu?.title)}
          </Link>
        ) : (
        ...
        {menu?.styleType == "image" && (
          <div
            className={`dropdown-menu dropdown-menu-2 dropdown-image ${
              !isOpen.length
                ? "show"
                : isOpen[level] === menu.title
                ? "show"
                : ""
            }`}
          >
